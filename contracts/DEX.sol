...
contract DEX is Ownable {
    ...
    function addLiquidityETH(
        ...
        if (keccak256(abi.encodePacked(tokenAddressToName[_token])) == keccak256(abi.encodePacked(""))) {
            ...
            liquidityProviderToLiquidityProvided[msg.sender][_token] =
                liquidityProviderToLiquidityProvided[msg.sender][_token] +
                deltaToken;
            liquidityProviderToLiquidityProvided[msg.sender][WETH] =
                liquidityProviderToLiquidityProvided[msg.sender][WETH] +
                msg.value;
    ...
    function removeLiquidityETH(
        ...
        require(
            liquidityProviderToLiquidityProvided[msg.sender][WETH] >= _ethAmount,
            "You have not provided this much liquidity for eth!"
        );
        require(
            liquidityProviderToLiquidityProvided[msg.sender][_token] >= _tokenAmount,
            "You have not provided this much liquidity for token!"
        );
        liquidityPoolToAvailableLiquidity[msg.sender][WETH] =
            liquidityPoolToAvailableLiquidity[msg.sender][WETH] -
            _ethAmount;
        liquidityPoolToAvailableLiquidity[msg.sender][_token] =
            liquidityPoolToAvailableLiquidity[msg.sender][_token] -
            _tokenAmount;
        lpAmounts memory amounts = liquidityPoolToAvailableLiquidity[WETH][_token];
        amounts.tokenA = amounts.tokenA - _ethAmount;
        amounts.tokenB = amounts.tokenB - _tokenAmount;
        liquidityPoolToAvailableLiquidity[WETH][_token] = amounts;
        tokenAddressToLiquidity[WETH] = tokenAddressToLiquidity[WETH] - _ethAmount;
        tokenAddressToLiquidity[_token] = tokenAddressToLiquidity[_token] - _tokenAmount;
    ...
    function ethToToken(address _token) public payable returns (uint256) {
        ...
        tokenAddressToLiquidity[_token] = tokenAddressToLiquidity[_token] - tokensBought;
        lpAmounts memory amounts = liquidityPoolToAvailableLiquidity[WETH][_token];
        amounts.tokenA = amounts.tokenA + msg.value;
        amounts.tokenB = amounts.tokenB - tokensBought;
        liquidityPoolToAvailableLiquidity[WETH][_token] = amounts;
    ...
    function tokenToEth(address _token, uint256 _tokenAmount) public returns (uint256) {
        ...
        require(
            ...
            "Token to ETH swap failed to send tokens!"
        );
        return ethBought;
    }
}
